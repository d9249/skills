...
2. Gather primary sources first
  ...
  - If the text mirror is blocked or returns 403 but the live site still renders in the browser, inspect the live page directly and also fetch the raw HTML. Product pages and official blogs often expose high-value structured metadata even when plain-text mirroring fails: `og:title`, `og:description`, `og:image`, canonical URL, article publish date, and `application/ld+json` blocks such as `SoftwareApplication`, `Organization`, or `Article`. Use those fields as grounded evidence for product positioning, feature lists, authorship, and official visuals.
   ...
   - If the shell lacks `pdftotext`, use Hermes `read_file` directly on the local PDF: it auto-extracts the text layer and can expose the full paper without installing a PDF CLI. Treat this as a text-layer extraction; use the arXiv HTML or rendered pages for figures, tables, and visual verification.
- For arXiv HTML figure asset collection and legibility checks, use `references/arxiv-html-figure-asset-verification.md` before converting figures into local blog assets.
...
7. Optional text-derived visual workflow
   ...
   - If the automated browser renderer is unavailable on macOS, rasterize SVGs with `sips -s format png <asset.svg> --out <preview.png>` for visual QA. Avoid Quick Look's fixed-size thumbnail output as the sole check because it can crop or distort the SVG aspect ratio.
   - Keep diagram labels short, manually wrap long Korean copy into separate lines, and size boxes around the longest rendered label rather than assuming the text will fit. For diagrams that must stay readable on the blog's roughly 290px-wide mobile article column, prefer a compact single-column/vertical layout with a moderate native width (around 600px) over a wide desktop infographic that merely scales down; verify the actual mobile render after Gatsby build.
...
- If the best official visual only exists as an extracted arXiv/source-tarball image, save a local optimized copy under `static/images/blog/<slug-or-topic>.webp` and embed it via `/images/blog/...`. Use Pillow, `ffmpeg`, or another deterministic converter, preserve legibility, and verify the asset is copied into `public/images/blog/` after a clean Gatsby build when cache staleness is possible. On macOS, `sips` may read but fail to write WebP (`Can't write format: org.webmproject.webp`); use `cwebp -q 90 <input.png> -o <output.webp>` when available, then check dimensions with `sips -g pixelWidth -g pixelHeight`.
- For paper figures with axes, legends, or embedded labels, compare the local optimized asset against the official arXiv HTML/PDF figure, not only against file size and HTTP status. If the official figure is readable but the local PNG/WebP becomes dark-dominant, loses labels, or appears to use the wrong matte/background, treat it as a collection/conversion failure and regenerate from the official HTML image or source bundle before relying on blog CSS.
- When using `scripts/arxiv_html_figure_contact_sheet.py`, inspect both `contact.jpg` and `figure_quality.json`. Any `dark_dominant_wide_figure` warning on a chart-like figure requires a manual source comparison before the image can be embedded.
...
- Preserve authored paragraph rhythm when satisfying citation coverage. Never insert blank lines after every cited sentence or mechanically turn a normal paragraph into one-sentence blocks just to raise a verifier score; this produces visibly fragmented Gatsby posts. Keep ordinary Korean body paragraphs at a natural 2–4 sentence cadence, attach citations to the supported sentence/paragraph, and if coverage is genuinely low, add concise source-grounded evidence rather than splitting existing prose.
- The current citation verifier splits sentences only when terminal punctuation is followed by whitespace. A citation written after the period (for example `문장이다.[2] 다음 문장`) can make two rendered sentences count as one verifier unit. Keep normal paragraph cadence while ensuring citation placement or soft line boundaries let the verifier recognize the supported sentence; re-run strict verification rather than adding arbitrary citations.
- Rendering QA is mandatory after the Gatsby build, not optional HTML-string inspection: inspect the built article at desktop and mobile widths in a real rendering engine or browser screenshot. Check for consecutive one-sentence micro-paragraphs, numeric citations orphaned at the start of a rendered line, overflow/sideways scrolling, unreadable table cells, images that escape their frame, caption separation, and source/sidebar overflow. Fix the Markdown/CSS cause and rebuild before staging. In this Gatsby renderer, bold Markdown around percentage-only tokens (for example `**49.37%**`) may appear literally as asterisks; inspect rendered number-heavy prose and use `<strong>49.37%</strong>` when that happens. If the Playwright MCP reports that Chrome for Testing is missing on macOS, first try `npx @playwright/mcp install-browser chrome-for-testing`. If it only prints a dependency warning without installing a browser, use the verified fallback `npx -y playwright@1.55.0 install chromium`, serve the built site locally, then capture desktop and mobile screenshots with `npx -y playwright@1.55.0 screenshot --viewport-size="<width>,<height>" --full-page <url> <output.png>`. Do not treat a static build as rendering verification.
