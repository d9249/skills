...
DEFAULT_SOURCE_REPO = Path("/Users/mean/Documents/Github/pp-ocr")
DEFAULT_TARGET_REPO = Path("/Users/mean/Documents/Github/ocr")
...
def find_source_repo(explicit: str | None) -> Path:
    ...
    for candidate in candidates:
        if candidate in seen:
            continue
        seen.add(candidate)
        mirror_script = candidate / "scripts" / "git" / "mirror_commit_to_ocr.py"
        if mirror_script.is_file() and (candidate / ".git").exists():
            return candidate
    raise SystemExit("Could not locate the pp-ocr source repo with scripts/git/mirror_commit_to_ocr.py")
...
def find_target_repo(source_repo: Path, explicit: str | None) -> Path:
    ...
    env_target = os.environ.get("PP_OCR_MIRROR_TARGET")
    if env_target:
        repo = Path(env_target).expanduser().resolve()
        if (repo / ".git").exists():
            return repo
        raise SystemExit(f"Target repo from PP_OCR_MIRROR_TARGET not found: {repo}")

    sibling = (source_repo.parent / "ocr").resolve()
    ...
    raise SystemExit("Could not locate the paired ocr target repo")
...
def ensure_commits_exist(source_repo: Path, commits: list[str]) -> None:
    for commit in commits:
        result = git(source_repo, "rev-parse", "--verify", commit, check=False)
...
def mirror_commits(
    ...
    mirror_script = source_repo / "scripts" / "git" / "mirror_commit_to_ocr.py"
    env = os.environ.copy()
    env["PP_OCR_MIRROR_TARGET"] = str(target_repo)
    env["PP_OCR_MIRROR_TARGET_BRANCH"] = target_branch
    for commit in commits:
        run([python_bin, str(mirror_script), commit], env=env)


def print_summary(source_repo: Path, target_repo: Path, target_branch: str, commits: list[str]) -> None:
    print("\n== target recent commits ==")
    git(target_repo, "log", "--oneline", "--decorate", "-n", str(max(5, len(commits) + 1)))
    ...
    last_success = source_repo / ".git" / "pp-ocr-mirror-last-success"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Mirror pp-ocr commits into the paired ocr repo and verify synced paths."
    )
    parser.add_argument("commits", nargs="+", help="Commit SHAs to mirror, in order")
    parser.add_argument("--source-repo", help="Path to the pp-ocr source repo")
    parser.add_argument("--target-repo", help="Path to the ocr target repo")
    parser.add_argument(
        "--target-branch",
        default=os.environ.get("PP_OCR_MIRROR_TARGET_BRANCH", "master"),
    ...
    parser.add_argument(
        ...
        help="Python executable used to call mirror_commit_to_ocr.py",
    )
